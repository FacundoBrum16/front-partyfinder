...
@Component({ components: { Modal, Slider } })
export default class SliderComponent extends Vue {
  @Prop({ default: () => [0, 100] })
  value!: number | number[]

  emitChange(val: number | number[]) {
...
.slider-target * {
  ...
  --slider-bg: #d1d5db;
  --slider-connect-bg: #60a5fa;
  --slider-connect-bg-disabled: #9ca3af;
  --slider-handle-ring-color: rgba(254, 205, 211, 0.3);
  --slider-height: 6px;
...
.slider-handle:focus {
  box-shadow: 0 0 0 var(--slider-handle-ring-width, 3px)
      var(--slider-handle-ring-color, #10b98130),
...
.slider-horizontal .slider-origin > .slider-tooltip-top {
  bottom: calc(
    var(--slider-tooltip-arrow-size, 5px) + var(--slider-handle-height, 16px) /
      2 - var(--slider-height, 6px) / 2 + var(--slider-tooltip-distance, 3px) +
      1px
  );
}
...
.slider-vertical .slider-origin > .slider-tooltip {
  ...
  transform: translateY(
    calc(
      var(--slider-tooltip-line-height, 1.25rem) * -1 -
        var(--slider-tooltip-py, 2px) * -1 + 1px
    )
  );
}
.slider-vertical .slider-origin > .slider-tooltip-left {
  right: calc(
    var(--slider-tooltip-arrow-size, 5px) + var(--slider-height, 6px) +
      var(--slider-handle-height, 16px) / 2 - var(--slider-height, 6px) / 2 +
