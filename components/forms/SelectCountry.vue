<template>
  <div>
    <label class="typo__label"><slot /></label>
    <t-rich-select
      class="text-white"
      :options="options"
      :value="computedValue"
    ...
    >
      <template #option="{ option }">
        ...
        >
          <div class="w-6 flex-shrink-0" v-html="parseEmoji(option.raw.flag)" />
      ...
      <template #label="{ option }">
        ...
        >
          ...
          <span class="font-bold truncate">
            {{ option.text }} ({{ option.raw.dialCode }})
          </span>
        </div>
      </template>
    </t-rich-select>
...
export default class SelectCountry extends Vue {
  ...
  get computedValue() {
    if (this.value) return this.value.code
  ...
  triggerSelect(val: string) {
    this.$emit(
      'select',
      this.options.find((opt) => opt.code === val)
    )
  }
